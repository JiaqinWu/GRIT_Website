                        ...
                        if submit_button:
                            if not new_note.strip():
                                ...
                                try:
                                    ...
                                    # Get headers - use get_all_values to see full structure including empty cells
                                    ...
                                    if not youth_entries.empty:
                                        ...
                                        # Add all other fields from existing row
                                        ...
                                        # Fill in empty values for other headers
                                        for header in sheet_headers:
                                            if header not in new_row_data:
                                                new_row_data[header] = ''
                        ...
                        if submit_button:
                            if not new_note.strip():
                                ...
                                try:
                                    ...
                                    # Use EXACT same method as "Add New Referral" - row_values(1) 
                                    # "Add New Referral" works, so we match it exactly
                                    sheet_headers = worksheet2.row_values(1)
                                    if not sheet_headers:
                                        st.error("❌ Unable to read sheet headers")
                                        st.stop()
                                    ...
                                    if not client_entries.empty:
                                        # Get existing row data using row_values - same as "Add New Referral" approach
                                        first_entry_idx = client_entries.index[0]
                                        sheet_row_num = first_entry_idx + 2  # +2: row 1 is header, DataFrame is 0-indexed
                                        existing_row_values = worksheet2.row_values(sheet_row_num)
                                        
                                        # Build dictionary - row_values() for both headers and data should align
                                        # since both skip empty leading cells the same way
                                        existing_data = {}
                                        # Ensure we have enough values (pad if row_values returned fewer columns)
                                        for i, header in enumerate(sheet_headers):
                                            if i < len(existing_row_values):
                                                existing_data[header] = existing_row_values[i] if existing_row_values[i] else ''
                                            else:
                                        ...
                                        # Build new_row_data dictionary - EXACT same structure as "Add New Referral"
                                        new_row_data = {
                                            'Day of Case Note': note_date_str,
                                            'Case Notes': new_note.strip(),
                                        ...
                                        # Convert to list in exact header order - NO prepending, just like "Add New Referral"
                                        new_row = [new_row_data.get(col, '') for col in sheet_headers]
                                    else:
                                        # If no previous entry exists
                                        new_row_data = {
