                        ...
                        if submit_button:
                            if not new_note.strip():
                                ...
                                try:
                                    ...
                                    # Headers start at column A, so row_values(1) will work correctly
                                    sheet_headers = worksheet2.row_values(1)
                                    if not sheet_headers:
                                        st.error("❌ Unable to read sheet headers")
                                        st.stop()
                                    ...
                                    if not client_entries.empty:
                                        ...
                                        # row_values returns values in same order as headers when both start from column A
                                        ...
                                        # Build dictionary from existing row - match by position
                                        # Since both row_values(1) and row_values(sheet_row_num) start from first non-empty cell,
                                        # and headers start at column A, they should align perfectly
                                        existing_data = {}
                                        for i, header in enumerate(sheet_headers):
                                            if i < len(existing_row_values):
